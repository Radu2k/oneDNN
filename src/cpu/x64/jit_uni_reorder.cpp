...
#define PARAM(x) ptr[abi_param1 + offsetof(call_param_t, x)]
        if (prb_.scale_type == scale_type_t::COMMON) {
            auto reg_ptr_scale_tmp = reg_ptr_in_;
            mov(reg_ptr_scale_tmp, PARAM(scale));
            uni_vbroadcastss(xmm_scale_, ptr[reg_ptr_scale_tmp]);
        } else if (prb_.scale_type == scale_type_t::MANY) {
...
private:
    ...
    Reg64 bf16_emu_scratch = reg_tmp_;
...
struct jit_single_blk_kernel_t : public jit_generator {
    ...
    void generate() override {
        ...
        if (prb_.req_src_zp) load_zp(ymm_src_zp_, reg_src_zp_);

        if (prb_.req_dst_zp) load_zp(ymm_dst_zp_, reg_dst_zp_);

        cmp(reg_ptr_tail_, true);
    ...
    void gen_setmask(int mask) {
        // all 0, all 1
        vxorps(ymm_tmp_, ymm_tmp_, ymm_tmp_);
        vpcmpeqd(ymm_mask_, ymm_mask_, ymm_mask_);
        // shift by mask to have tail nelems in ymm_mask_
        const uint8_t in_mask = 0xFF << mask;
        vpblendd(ymm_mask_, ymm_mask_, ymm_tmp_, in_mask);
    }
        ...
        for (int i = 0; i < out_tail; ++i) {
            if (in_tail != lane) {
                gen_maskloadu(Ymm(i),
                        ...
                        ymm_mask_, lane * itype_sz_);
            } else {
                gen_loadu(Ymm(i),
                        ptr[reg_ptr_in_ + i_off + i * input_stride * itype_sz_],
                        lane * itype_sz_);
            }
            if (prb_.req_src_zp) { vsubps(Ymm(i), Ymm(i), ymm_src_zp_); }
        }

        gen_transpose_8x8();

        for (int i = 0; i < in_tail; ++i) {
            if (prb_.req_dst_zp) { vaddps(Ymm(i), Ymm(i), ymm_dst_zp_); }
            if (out_tail == lane) {
                ...
                gen_maskstoreu(ptr[reg_ptr_out_ + o_off
                                       + i * output_stride * otype_sz_],
                        Ymm(i), ymm_mask_, lane * otype_sz_);
            }
        }
    }
...
private:
    ...
    void preamble() {
        if (is_windows) {
            ...
            mov(reg_dst_zp_, ptr[rsp + param5]);
    ...
    Reg8 reg_ptr_tail_ = is_windows ? r8b : dl;
    Reg64 reg_src_zp_ = abi_param4;
    Reg64 reg_dst_zp_ = is_windows ? r10 : r8;

    Ymm ymm_mask_ = ymm12;
    Ymm ymm_tmp_ = ymm0;
    Ymm ymm_src_zp_ = ymm14;
    Ymm ymm_dst_zp_ = ymm15;
};
