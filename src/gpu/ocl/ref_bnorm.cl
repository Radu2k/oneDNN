...
__kernel void ref_bnorm_fwd(__global DATA_T *src, __global float *mean,
    ...
    const VECT_FLOAT_T sm = AS_VECT_FLOAT_T(
            VECT_UINT_READ((const __global uint *)(scaleshift + c)));
    const VECT_FLOAT_T sv = AS_VECT_FLOAT_T(
            VECT_UINT_READ((const __global uint *)(scaleshift + c + IC)));
#else
    const float sm = 1.0f;
    const float sv = 0.0f;
#endif
    ...
    VECT_FLOAT_T v_mean
            = AS_VECT_FLOAT_T(VECT_UINT_READ((const __global uint *)&mean[c]));
    VECT_FLOAT_T v_variance = AS_VECT_FLOAT_T(
            VECT_UINT_READ((const __global uint *)&variance[c]));
