...
struct gen_gemm_t : public gpu_gemm_t {
    ...
    struct pd_t : public gpu_gemm_pd_t {
        ...
        status_t init(engine_t *engine) {
            ...
            // int8 not enabled on Xe_HP+ for now. bf16 only enabled on Xe_HP+.
            ok &= IMPLICATION(
                    is_xe_hp_plus, utils::one_of(d->c_type(), f16, bf16, f32));
            ok &= IMPLICATION(d->c_type() == bf16, is_xe_hp_plus);
#endif
            if (!ok) return status::unimplemented;
