...
struct jit_gen12lp_gemm_t : public primitive_impl_t {
    ...
    status_t init_nocopy() {
        ...
        for (bool aligned : {false, true}) {
            for (bool aoffnonzero : {false, true}) {
                for (bool boffnonzero : {false, true}) {
                    compute::kernel_ctx_t kernel_ctx;

                    auto status = jit_gen12lp_gemm_x8x8s32_kernel<a_type,
                            b_type, c_type>::init_const_def(kernel_ctx,
                            pd()->desc()->transa, pd()->desc()->transb, fixed_c,
                            column_c, row_c, pd()->with_eltwise(),
                            pd()->eltwise_alg_kind(), aligned, aoffnonzero,
                            boffnonzero);
                    if (status != status::success) return status;

                    compute_engine->create_kernel(
                            &compute_x8x8s32_kernel_[aligned][aoffnonzero]
                                                    [boffnonzero],
                            kernel_name, kernel_ctx);
                    if (!compute_x8x8s32_kernel_[aligned][aoffnonzero]
                                                [boffnonzero])
    ...
    jit_gen12lp_gemm_t(const pd_t *apd) : primitive_impl_t(apd) {}
...
private:
    status_t launch_x8x8s32(compute::compute_stream_t *s,
            ...
            bool aligned, bool aoffnonzero, bool boffnonzero) const;
    ...
    compute::kernel_t compute_x8x8s32_kernel_[2][2][2];
