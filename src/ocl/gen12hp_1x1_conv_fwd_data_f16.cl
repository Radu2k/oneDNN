...
#define BLOCK_READ_WHT(data, idx) \
    data = as_half8( \
            intel_sub_group_block_read_us8((__global ushort *)&wei[idx]));
...
inline half8 POST_OPS_PASS(half8 val, half2 bias, float scales, float alpha,
        float beta, float sum_scale, __global half *dst) {
#if WITH_BIAS
    val[0] += bias[0] * (half)scales;
    val[2] += bias[0] * (half)scales;
    val[4] += bias[0] * (half)scales;
    val[6] += bias[0] * (half)scales;
    val[1] += bias[1] * (half)scales;
    val[3] += bias[1] * (half)scales;
    val[5] += bias[1] * (half)scales;
    val[7] += bias[1] * (half)scales;
#else
    val *= (half)scales;
#endif // WITH_BIAS

#if WITH_ELTWISE == 1
    val[0] = fwd_eltwise(val[0], (half)alpha, (half)beta);
    val[1] = fwd_eltwise(val[1], (half)alpha, (half)beta);
    val[2] = fwd_eltwise(val[2], (half)alpha, (half)beta);
    val[3] = fwd_eltwise(val[3], (half)alpha, (half)beta);
    val[4] = fwd_eltwise(val[4], (half)alpha, (half)beta);
    val[5] = fwd_eltwise(val[5], (half)alpha, (half)beta);
    val[6] = fwd_eltwise(val[6], (half)alpha, (half)beta);
    val[7] = fwd_eltwise(val[7], (half)alpha, (half)beta);
#endif // WITH_ELTWISE

#if WITH_SUM
    val += as_half8(intel_sub_group_block_read_us8((__global ushort *)&dst[0]))
            * (half)sum_scale;
#endif // WITH_SUM
...
gen12hp_1x1_conv_fwd_kernel_f16(const __global half *src,
    ...
    for (uint ic_block_id = 0; ic_block_id < IC_NCHUNK; ++ic_block_id) {
        ...
        // MB 0-7, OC sg_lid
        C00 = MMAD8X8(S0, as_int8((half16)(W00, W01)), C00);
        // MB 0-7, OC sg_lid + sg_size
        C10 = MMAD8X8(S0, as_int8((half16)(W10, W11)), C10);
        // MB 8-15, OC sg_lid
        C01 = MMAD8X8(S1, as_int8((half16)(W00, W01)), C01);
        // MB 8-15, OC sg_lid + sg_size
        C11 = MMAD8X8(S1, as_int8((half16)(W10, W11)), C11);
        // MB 16-23, OC sg_lid
        C02 = MMAD8X8(S2, as_int8((half16)(W00, W01)), C02);
        // MB 16-23, OC sg_lid + sg_size
        C12 = MMAD8X8(S2, as_int8((half16)(W10, W11)), C12);
        // MB 24-31, OC sg_lid
        C03 = MMAD8X8(S3, as_int8((half16)(W00, W01)), C03);
        // MB 24-31, OC sg_lid + sg_size
        C13 = MMAD8X8(S3, as_int8((half16)(W10, W11)), C13);
...
#define PACK_AND_WRITE(_acc0, _acc1) \
    dst_val[0] = (half8)(_acc0[0], _acc1[0], _acc0[1], _acc1[1], _acc0[2], \
            _acc1[2], _acc0[3], _acc1[3]); \
    dst_val[1] = (half8)(_acc0[4], _acc1[4], _acc0[5], _acc1[5], _acc0[6], \
            _acc1[6], _acc0[7], _acc1[7]); \
\
    dst_val[0] = POST_OPS_PASS(dst_val[0], bias_val, scales, eltwise_alpha, \
            eltwise_beta, sum_scale, dst); \
    dst_val[1] = POST_OPS_PASS(dst_val[1], bias_val, scales, eltwise_alpha, \
            eltwise_beta, sum_scale, dst + 64); \
\
    intel_sub_group_block_write_us8( \
            (__global ushort *)&dst[0], as_ushort8(dst_val[0])); \
    dst += 64; \
    intel_sub_group_block_write_us8( \
            (__global ushort *)&dst[0], as_ushort8(dst_val[1])); \
    dst += 64;
