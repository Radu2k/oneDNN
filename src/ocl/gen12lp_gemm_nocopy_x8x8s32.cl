...
#ifdef	FF
#define	ADD_EACH(X, OFF) do { \
    if (n > X + OFF) { \
        if (m > 0) c[0] = ((!beta)? 0 : c[0]) + sc[X / 4 +  0].s ## OFF + ((!apply_co)? 0 : co[0]); \
        if (m > 1) c[1] = ((!beta)? 0 : c[1]) + sc[X / 4 +  4].s ## OFF + ((!apply_co)? 0 : co[0]); \ 
        if (m > 2) c[2] = ((!beta)? 0 : c[2]) + sc[X / 4 +  8].s ## OFF + ((!apply_co)? 0 : co[0]); \
        if (m > 3) c[3] = ((!beta)? 0 : c[3]) + sc[X / 4 + 12].s ## OFF + ((!apply_co)? 0 : co[0]); \
        c  += ldc; \
    } \
} while(0)
#elif	defined CC
#define	ADD_EACH(X, OFF) do { \
    if (n > X + OFF) { \
        if (m > 0) c[0] = ((!beta)? 0 : c[0]) + sc[X / 4 +  0].s ## OFF + ((!apply_co)? 0 : co[0]); \
        if (m > 1) c[1] = ((!beta)? 0 : c[1]) + sc[X / 4 +  4].s ## OFF + ((!apply_co)? 0 : co[1]); \
        if (m > 2) c[2] = ((!beta)? 0 : c[2]) + sc[X / 4 +  8].s ## OFF + ((!apply_co)? 0 : co[2]); \
        if (m > 3) c[3] = ((!beta)? 0 : c[3]) + sc[X / 4 + 12].s ## OFF + ((!apply_co)? 0 : co[3]); \
        c  += ldc; \
    } \
} while(0)
#else
...
#define	ACCUMULATE_1(a, b) \
    ((FLOATC)a.s0 * (FLOATC)b.s0) + ((FLOATC)a.s1 * (FLOATC)b.s1) + ((FLOATC)a.s2 * (FLOATC)b.s2) + ((FLOATC)a.s3 * (FLOATC)b.s3)

#define	ACCUMULATE(a, b0, b1, b2, b3) \
    (FLOATC4)(ACCUMULATE_1(a, b0), ACCUMULATE_1(a, b1), ACCUMULATE_1(a, b2), ACCUMULATE_1(a, b3))
...
__attribute__((intel_reqd_sub_group_size(GRX)))
kernel void gen12lp_gemm_compute_x8x8s32_kernel( global FLOATA *a, global FLOATB *b, global FLOATC *c, long offsetA, long offsetB, long offsetC, long lda, long ldb, long ldc, long m, long n, long k, int beta, FLOATA ao, FLOATB bo, global FLOATC *co, long offsetCO, int apply_co, local FLOATA *sa, local FLOATB *sb, int apply_eltwise, float eltwise_alpha, float eltwise_beta) {
    ...
    long moffset = ((cid * mwidth) & ~(UNROLL_M - 1)) * kk + ((cid * mwidth) & (UNROLL_M - 1)) * UNROLL_K;
    long noffset = ((cid * nwidth) & ~(UNROLL_N - 1)) * kk + ((cid * nwidth) & (UNROLL_N - 1)) * UNROLL_K;
    ...
    for (l = 0; l < kk; l += UNROLL_K) {
        for (ll = 0; ll < UNROLL_K; ll ++) {
            sa[moffset + l * UNROLL_M + ll] = (((cid < m) && (l + ll < k))? *a - ao : 0);
#if defined(NN) || defined(NT)
            a += lda;
#else
            a ++;
    ...
    for (l = 0; l < kk; l += UNROLL_K) {
        for (ll = 0; ll < UNROLL_K; ll ++) {
            sb[noffset + l * UNROLL_N + ll] = (((cid < n) && (l + ll < k))? *b - bo : 0);
#if defined(NN) || defined(TN)
            b ++;
#else
            b += ldb;
    ...
    m -= GROUPSIZE_M * gdx + UNROLL_M * idx;  if (m > UNROLL_M) m = UNROLL_M;
    n -= GROUPSIZE_N * gdy + UNROLL_N * idy;  if (n > UNROLL_N) n = UNROLL_N;

    c  += offsetC + UNROLL_M * idx + GROUPSIZE_M * gdx + UNROLL_M * lid / GRX + (UNROLL_N * idy + GROUPSIZE_N * gdy) * ldc;
