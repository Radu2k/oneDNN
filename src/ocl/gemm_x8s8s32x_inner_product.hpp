...
inline status_t create_gemm_x8s8s32x_pd(primitive_desc_t **gemm_pd,
    ...
    return mkldnn_primitive_desc_create(gemm_pd, (op_desc_t *)&gemm_desc, &attr,
            engine, nullptr);
}
}
...
struct gemm_x8s8s32x_inner_product_fwd_t : public primitive_impl_t {
    struct pd_t : public ocl_inner_product_fwd_pd_t {
        ...
        DECLARE_COMMON_PD_T("ocl:gemm_x8s8s32x",
        ...
        status_t init() {
            ...
            bool gemm_ok = status::success == create_gemm_x8s8s32x_pd(&gemm_pd_,
                    this->engine(),
